...
local AlwaysAllow = {
	-- spells could get stuck but it's very rare, this table is for that
...
local function ShouldSkipAura(frame, event, unit, updateInfo, shouldDisplay)
	...
	if not added then
		return false
	end -- a new aura has appeared
	if not updated then
		return false
	end -- an existing aura has been altered
	if not removed then
		return false
	end -- an aura has been yeeted into the abyss
