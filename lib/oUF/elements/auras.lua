	...
	* show - indicates whether the aura button should be shown (boolean)
	...
	if (show) then
		...
		if button.Stealable then
			button.Stealable:SetShown(
				not isDebuff and isStealable and element.showStealableBuffs and not UnitIsUnit("player", unit)
			)
		end
...
local function filterIcons(element, unit, filter, limit, isDebuff, offset, dontHide)
	...
	if not unitAuraInfo then
		unitAuraInfo = {}
		AuraInfo[unit] = unitAuraInfo
	end
	local auraInstanceID, aura = next(unitAuraInfo)
	while (aura or forceShow) and (visible < limit) do
		local result =
			(forceShow or not oUF:ShouldSkipAuraFilter(aura, filter)) and
			updateAura(element, unit, aura, index, offset, filter, isDebuff, visible)
...
local function UpdateAuras(self, event, unit, updateInfo)
	...
	if (auras) then
		...
		* unit - the unit for which the update has been triggered (string)
		...
		local visibleBuffs =
			filterIcons(auras, unit, auras.buffFilter or auras.filter or "HELPFUL", min(numBuffs, maxAuras), nil, 0, true)
		...
		local visibleDebuffs =
			filterIcons(
				...
				min(numDebuffs, maxAuras - visibleBuffs),
				true,
				visibleBuffs
			)
	...
	if (debuffs) then
		...
		if (fromRange or debuffs.createdButtons > debuffs.anchoredButtons) then
			(debuffs.SetPosition or SetPosition)(
					debuffs,
					fromRange or debuffs.anchoredButtons + 1,
					toRange or debuffs.createdButtons
				)
