...
local function initialize()
	...
	attributes.maxColumns = pets
	attributes.columnAnchorPoint = new_group_anchor
	attributes['initial-width'] = config.pets_width
	attributes['initial-height'] = config.pets_height
	attributes.point = new_player_anchor
	attributes['oUF-initialConfigFunction'] = format('self:SetWidth(%d); self:SetHeight(%d);', config.pets_width, config.pets_height)

	-- ouf gives us a secureheader for attributes
	mod.petframeHeader = oUF:SpawnHeader(nil, "SecureGroupPetHeaderTemplate", 'raid', attributes)
...
local function callback()
	if (not mod.petframeHeader) then
		return
	end
	update_pet_header()
end
